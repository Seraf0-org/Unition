...
namespace Unition.Editor
{
    ...
    public class NotionConfigEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Generate Keys Class button
            if (config.databaseMappings.Count > 0)
            {
                EditorGUILayout.Space(5);
                if (GUILayout.Button("Generate Keys Class"))
                {
                    GenerateKeysClass(config);
                }
            }
        ...
        private void GenerateKeysClass(NotionConfig config)
        {
            // Ask for save location
            string path = EditorUtility.SaveFilePanelInProject(
                "Save Database Keys Class",
                "NotionDbKeys",
                "cs",
                "Choose where to save the generated keys class"
            );
            ...
            sb.AppendLine("// Auto-generated by Unition. Do not edit manually.");
            sb.AppendLine("// Regenerate from NotionConfig Inspector if mappings change.");
            sb.AppendLine();
            sb.AppendLine("namespace Unition.Generated");
            sb.AppendLine("{");
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// Database mapping keys for NotionConfig.GetDatabaseId().");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    public static class NotionDbKeys");
            sb.AppendLine("    {");
            
            foreach (var mapping in config.databaseMappings)
            {
                if (string.IsNullOrEmpty(mapping.key)) continue;
                
                // Convert key to valid C# identifier (PascalCase)
                string identifier = ToPascalCase(mapping.key);
                
                sb.AppendLine($"        /// <summary>Key for database: {mapping.databaseName}</summary>");
                sb.AppendLine($"        public const string {identifier} = \"{mapping.key}\";");
            }
            ...
            Debug.Log($"[Unition] Generated keys class at: {path}");
            EditorUtility.DisplayDialog("Keys Generated", $"Database keys class generated at:\n{path}", "OK");
        }
